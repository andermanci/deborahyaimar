...
const tabActiva = await page.locator('.gl-tab.active').getAttribute('data-cat');
tabActiva === 'invitados' ? ok('abre en la pestaña Invitados, no en una vacía') : mal(`abre en "${tabActiva}"`);

await page.waitForSelector('.gl-item', { timeout: 5000 });
const n = await page.locator('.gl-item').count();
n === 2 ? ok(`pinta las ${n} fotos del índice`) : mal(`pinta ${n} fotos, esperaba 2`);

// ── 2. XSS ───────────────────────────────────────────────────────────
console.log('\n2) Nombre malicioso de invitado');
const inyectadas = await page.locator('.gl-item img[src*="onerror"], .gl-item-name img').count();
inyectadas === 0 ? ok('el HTML del nombre NO se ejecuta') : mal('¡se inyectó HTML del nombre!');
const textoNombre = await page.locator('.gl-item-name').first().textContent();
textoNombre.includes('<img') ? ok('el nombre se muestra como texto plano') : mal(`nombre inesperado: ${textoNombre}`);

// ── 3. Vídeo ─────────────────────────────────────────────────────────
console.log('\n3) Tarjeta de vídeo');
await page.locator('.gl-play').count() === 1 ? ok('el vídeo lleva icono de play') : mal('falta el icono de play');
const dur = await page.locator('.gl-dur').textContent();
dur === '12s' ? ok('muestra la duración') : mal(`duración: ${dur}`);

// ── 4. Lightbox y swipe ──────────────────────────────────────────────
console.log('\n4) Lightbox');
await page.locator('.gl-item').first().click();
await page.waitForSelector('.lightbox.open', { timeout: 3000 });
ok('abre al tocar una foto');
const srcAntes = await page.locator('#lbSlot img').getAttribute('src');
...
const hayVideo = await page.locator('#lbSlot video').count();
hayVideo === 1 ? ok('el swipe avanza a la siguiente (el vídeo)') : mal('el swipe no avanzó');
await page.keyboard.press('Escape');
await page.locator('.lightbox.open').count() === 0 ? ok('Escape cierra') : mal('Escape no cierra');
...
await page.waitForFunction(() => document.getElementById('ufColaTexto')?.textContent?.includes('Gracias'), { timeout: 20000 });
...
console.log('\n6) El nombre se recuerda');
await page.reload({ waitUntil: 'networkidle' });
const recordado = await page.inputValue('#ufNombre');
recordado === 'Ander' ? ok('el nombre sigue puesto tras recargar') : mal(`recordó "${recordado}"`);
